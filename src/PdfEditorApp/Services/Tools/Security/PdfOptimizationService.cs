...
public interface IPdfOptimizationService
{
    ...
    Task<ToolExecutionResult> LinearizePdfAsync(LinearizeToolOptions options, IProgress<double>? progress = null, CancellationToken ct = default);
}
...
public class PdfOptimizationService : IPdfOptimizationService
{
    ...
    public PdfOptimizationService(IQuestPdfOperationsEngine? questEngine = null)
    {
    ...
    public async Task<ToolExecutionResult> CompressPdfAsync(CompressToolOptions options, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        return await Task.Run(() =>
        {
            ...
            // High-Performance QuestPDF Web Linearization (Fast Web View)
            if (options.LinearizeForWeb && File.Exists(outPath))
            {
                var linResult = _questEngine.LinearizeAsync(outPath, outPath, null, ct).GetAwaiter().GetResult();
                if (linResult.Success)
                {
                    outBytes = new FileInfo(outPath).Length;
                    message += " Optimized for Fast Web View (Linearized).";
                }
            }
    ...
    public Task<ToolExecutionResult> LinearizePdfAsync(LinearizeToolOptions options, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        return _questEngine.LinearizeAsync(options.InputFilePath, options.OutputFilePath, progress, ct);
    }
