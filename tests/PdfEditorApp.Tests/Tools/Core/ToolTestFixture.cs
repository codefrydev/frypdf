...
namespace PdfEditorApp.Tests.Tools.Core;
...
public class ToolTestFixture
{
    ...
    public static string CreateIsolatedDirectory(string prefix = "PdfTest")
    {
    ...
    /// </summary>
    public static string CreateSamplePdf(string name = "Sample", int pageCount = 2, string? directory = null)
    {
        string dir = directory ?? Path.GetTempPath();
        string path = Path.Combine(dir, $"{name}_{Guid.NewGuid():N}.pdf");
    ...
    /// </summary>
    public static string CreatePaddedWebPdf(string name = "WebPaddedSample", int pageCount = 3, string? directory = null)
    {
        string basePdf = CreateSamplePdf(name, pageCount, directory);
        ...
        string dir = directory ?? Path.GetTempPath();
        string paddedPath = Path.Combine(dir, $"{name}_padded_{Guid.NewGuid():N}.pdf");
