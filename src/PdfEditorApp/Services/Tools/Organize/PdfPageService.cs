...
namespace PdfEditorApp.Services.Tools.Organize;
...
public interface IPdfPageService
{
    ...
    Task<ToolExecutionResult> ApplyPdfLayerAsync(string inputPath, string layerPdfPath, bool isOverlay, string targetPages, string? outputPath = null, IProgress<double>? progress = null, CancellationToken ct = default);
}
...
public class PdfPageService : IPdfPageService
{
    ...
    public PdfPageService(IQuestPdfOperationsEngine? questEngine = null)
    {
    ...
    public async Task<ToolExecutionResult> MergePdfAsync(MergeToolOptions options, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        ...
        {
            var questResult = await _questEngine.MergeAsync(options, progress, ct);
            if (questResult.Success)
                return questResult;

            System.Diagnostics.Debug.WriteLine($"QuestPDF Merge fallback: {questResult.ErrorMessage}");
        }

        return await MergeWithPdfSharpAsync(options, progress, ct);
    }

    private async Task<ToolExecutionResult> MergeWithPdfSharpAsync(MergeToolOptions options, IProgress<double>? progress, CancellationToken ct)
    {
    ...
    public Task<ToolExecutionResult> ApplyPdfLayerAsync(string inputPath, string layerPdfPath, bool isOverlay, string targetPages, string? outputPath = null, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        return _questEngine.ApplyLayerAsync(inputPath, layerPdfPath, isOverlay, targetPages, outputPath, progress, ct);
    }
}
